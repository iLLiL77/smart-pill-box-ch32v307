/**********************************************
 * MAX30102 心率血氧传感器驱动 — 头文件 (STM32F103C8T6)
 *
 * I2C: PB7(SCL) + PB8(SDA), 软件I2C, 地址 0x57
 * INT: PB9 (上拉输入, FIFO数据就绪=低电平)
 * 算法: Maxim Integrated 官方算法 (汉明窗+峰值检测+SpO2查表)
 *
 * 使用:
 *   MAX30102_Init();              // 初始化, 返回1=成功 0=传感器未找到
 *   MAX30102_Update();            // 主循环中每~100ms调用一次
 *   hr  = MAX30102_GetHR();       // 读心率 BPM
 *   sp  = MAX30102_GetSpO2();     // 读血氧 %
 *   ok  = MAX30102_IsValid();     // 数据有效?
 *   fg  = MAX30102_FingerOn();    // 手指在传感器上?
 *   q   = MAX30102_GetSignalQuality(); // 信号质量 0~100
 *
 * 移植自 CH32V307VCT 成熟工程, 适配 STM32F103C8T6
 **********************************************/
#ifndef __MAX30102_H
#define __MAX30102_H
...
#define MAX30102_READ_SDA    PBin(8)
